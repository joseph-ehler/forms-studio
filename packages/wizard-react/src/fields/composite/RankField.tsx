...
export const RankField: React.FC<FieldComponentProps> = ({
  ...
  const helper = config.description
  ...
  const options: RankOption[] = Array.isArray(json?.options) ? json!.options : []
  const showRankNumbers = Boolean(json?.showRankNumbers ?? true)
  const instructions = String(json?.instructions ?? 'Drag to reorder items by preference')
  ...
  return (
    <Stack spacing="lg">
      ...
      {typography.showDescription && helper && (
        <div className="mb-3">
      ...
      <Controller
        ...
        render={({ field }) => (
          <RankList
            value={Array.isArray(field.value) ? field.value : defaultOrder}
...
const RankList: React.FC<RankListProps> = ({
  ...
  const rankedItems: RankOption[] = (value.length ? value : options.map(o => o.id))
    .map(id => options.find(o => o.id === id))
    .filter(Boolean) as RankOption[]

  const handleDragStart = (index: number) => {
    if (!disabled) setDraggedIndex(index)
  }
